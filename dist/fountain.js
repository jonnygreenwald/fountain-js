...
var Fountain = /** @class */ (function () {
    ...
    Fountain.prototype.parser = function (script) {
        this._tokens = new scanner_1.Scanner().tokenize(script);
        var token, i = this._tokens.length;
        while (i--) {
            token = this._tokens[i];
            /**
             * TODO: token.text = inline.lexer(token.text);
             */
            switch (token.type) {
                case 'title':
        ...
        return { title: this.title, html: { title_page: this.title_page.join(''), script: this.html.join('') } };
    };
    Object.defineProperty(Fountain.prototype, "tokens", {
        get: function () {
            return this._tokens.reverse();
        },
        enumerable: true,
        configurable: true
    });
